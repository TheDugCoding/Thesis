...
def model_list_rq3_ex1(data, n_samples_train):
    ...
    n_samples_val = n_samples_train // 10
    ...
    filepath = os.path.join(models_finetuning_path,
                            f'framework_complex_finetuning_gin_train_set_size_{n_samples_train}.txt')
    hyperparams_framework_gin = parse_hyperparams(filepath)

    train_loader_gnn_model_complex_framework_gin_without_front_flex = NeighborLoader(
        data,
        shuffle=True,
        num_neighbors=hyperparams_framework_gin.get('neighbours_size'),
        batch_size=64,
        input_nodes=train_mask
    )
    ...
    gnn_model_downstream_framework_without_flipping_layer = GIN(
        in_channels=data.num_features + 128,
        hidden_channels=hyperparams_framework_gin.get('hidden_channels'),
        num_layers=hyperparams_framework_gin.get('num_layers'),
        out_channels=2,
        dropout=hyperparams_framework_gin.get('dropout'),
        act=hyperparams_framework_gin.get('act'),
        norm=get_norm(hyperparams_framework_gin.get('norm'), hyperparams_framework_gin.get('hidden_channels'))
    )

    gnn_model_complex_framework_gin_without_front_flex = DGIPlusGNN(dgi_model_without_flipping_layer,
                                                                    gnn_model_downstream_framework_without_flipping_layer,
                                                                    False)
    optimizer_gnn_complex_framework_gin_without_front_flex = torch.optim.Adam(gnn_model_complex_framework_gin_without_front_flex.parameters(), lr=hyperparams_framework_gin.get('lr')
                                                , weight_decay=hyperparams_framework_gin.get('weight_decay'))
    ...
    filepath = os.path.join(models_finetuning_path,
                            f'framework_simple_finetuning_train_set_size_{n_samples_train}.txt')
    hyperparams_simple_framework_gin = parse_hyperparams(filepath)

    train_loader_gnn_model_simple_framework_gin_without_front_flex = NeighborLoader(
        data,
        shuffle=True,
        num_neighbors=hyperparams_simple_framework_gin.get('neighbours_size'),
        batch_size=64,
        input_nodes=train_mask
    )
    ...
    gnn_model_downstream_simple_framework_gin = GIN(
        in_channels=data.num_features,
        hidden_channels=hyperparams_simple_framework_gin.get('hidden_channels'),
        num_layers=hyperparams_simple_framework_gin.get('num_layers'),
        out_channels=hyperparams_simple_framework_gin.get('output_channels'),
        dropout=hyperparams_simple_framework_gin.get('dropout'),
        act=hyperparams_simple_framework_gin.get('act'),
        norm=get_norm(hyperparams_simple_framework_gin.get('norm'), hyperparams_simple_framework_gin.get('hidden_channels'))
    )

    layer_sizes = [128 + hyperparams_simple_framework_gin.get('output_channels')] + [hyperparams_simple_framework_gin.get('hidden_channels_mlp')] * hyperparams_simple_framework_gin.get('num_layers_mlp') + [2]
    mlp = build_mlp(layer_sizes, activation_fn, hyperparams_simple_framework_gin.get('dropout_mlp'))
    ...
    optimizer_gnn_simple_framework_gin_without_front_flex = torch.optim.Adam(
        ...
        lr=hyperparams_simple_framework_gin.get('lr'),
        weight_decay=hyperparams_simple_framework_gin.get('weight_decay'))
    ...
    filepath = os.path.join(models_finetuning_path,
                            f'graphsage_finetuning_all_features_train_set_size_{n_samples_train}.txt')
    hyperparams_simple_framework_graphsage_all_features = parse_hyperparams(filepath)

    train_loader_gnn_all_features_graphsage = NeighborLoader(
        data_all_features,
        shuffle=True,
        num_neighbors=hyperparams_simple_framework_graphsage_all_features.get('neighbours_size'),
        batch_size=32,
    ...
    gnn_model_all_features_graphsage = GraphSAGE(
        ...
        hidden_channels=hyperparams_simple_framework_graphsage_all_features.get('hidden_channels'),
        num_layers=hyperparams_simple_framework_graphsage_all_features.get('num_layers'),
        out_channels=2,
        dropout=hyperparams_simple_framework_graphsage_all_features.get('dropout'),
        act=hyperparams_simple_framework_graphsage_all_features.get('act'),
        aggr=hyperparams_simple_framework_graphsage_all_features.get('aggr'),
        norm=get_norm(hyperparams_simple_framework_graphsage_all_features.get('norm'), hyperparams_simple_framework_graphsage_all_features.get('hidden_channels'))
    )
    optimizer_gnn_all_features_graphsage = torch.optim.Adam(gnn_model_all_features_graphsage.parameters(),
                                                            lr=hyperparams_simple_framework_graphsage_all_features.get('lr'),
                                                            weight_decay=hyperparams_simple_framework_graphsage_all_features.get('weight_decay'))
