...
from src.modeling.pre_training.topological_pre_training.deep_graph_infomax_only_topological_features import dgi_original_graphsage, dgi_original_graphsage_only_degree, dgi_original_gin, infonce_graphsage
...
def train_once(model, train_data, neighbours_size, batch_size, optimizer, criterion):
    model.train()
...
def test_once(model, test_data, neighbours_size, batch_size, mask_attr='test_mask'):
    model.eval()
    true = []
    probs = []
...
def objective_framework_complex_only_degree_dgi(trial):
    ...
    # define the framework, first DGI and then the GNN used in the downstream task
    dgi_model_without_flipping_layer = DeepGraphInfomaxWithoutFlexFronts(
        hidden_channels=dgi_original_graphsage_only_degree["output_channels"],
        encoder=EncoderWithoutFlexFrontsGraphsage(
            input_channels=dgi_original_graphsage_only_degree["data_rabo"].num_features,
            hidden_channels=dgi_original_graphsage_only_degree["hidden_channels"],
            output_channels=dgi_original_graphsage_only_degree["output_channels"],
            layers=dgi_original_graphsage_only_degree["num_layers"],
            activation_fn=dgi_original_graphsage_only_degree["act"]
        ),
        summary=lambda z, *args, **kwargs: torch.sigmoid(z.mean(dim=0)),
        corruption=corruption_without_flex_fronts)
    # load the pretrained parameters
    dgi_model_without_flipping_layer.load_state_dict(
        torch.load(
            os.path.join(trained_dgi_model_path,
                         dgi_original_graphsage_only_degree['pth_name'])))
...
def objective_framework_complex_gin_encoder(trial):
    ...
    # define the framework, first DGI and then the GNN used in the downstream task
    dgi_model_without_flipping_layer = DeepGraphInfomaxWithoutFlexFronts(
        hidden_channels=dgi_original_gin["output_channels"],
        encoder=EncoderWithoutFlexFrontsGIN(
            input_channels=dgi_original_gin["data_rabo"].num_features,
            hidden_channels=dgi_original_gin["hidden_channels"],
            output_channels=dgi_original_gin["output_channels"],
            layers=dgi_original_gin["num_layers"],
            activation_fn=dgi_original_gin["act"]),
        summary=lambda z, *args, **kwargs: torch.sigmoid(z.mean(dim=0)),
        corruption=corruption_without_flex_fronts)
    # load the pretrained parameters
    dgi_model_without_flipping_layer.load_state_dict(
...
def objective_framework_complex_infonce(trial):
    ...
    # define the framework, first DGI and then the GNN used in the downstream task
    dgi_model_without_flipping_layer = DeepGraphInfomaxWithoutFlexFronts(
        hidden_channels=infonce_graphsage["output_channels"],
        encoder=EncoderWithoutFlexFrontsGraphsage(
            input_channels=infonce_graphsage["data_rabo"].num_features,
            hidden_channels=infonce_graphsage["hidden_channels"],
            output_channels=infonce_graphsage["output_channels"],
            layers=infonce_graphsage["num_layers"],
            activation_fn=infonce_graphsage["act"]),
        summary=lambda z, *args, **kwargs: torch.sigmoid(z.mean(dim=0)),
        corruption=corruption_without_flex_fronts)
    # load the pretrained parameters
    dgi_model_without_flipping_layer.load_state_dict(
...
def objective_framework_simple(trial):
    ...
    layer_sizes = [dgi_original_graphsage["output_channels"] + output_channels] + [hidden_channels_mlp] * num_mlp_layers + [2]
...
def objective_framework_simple_only_degree_dgi(trial):
    ...
    # Define MLP layers for classification
    layer_sizes = [dgi_original_graphsage_only_degree["output_channels"] + output_channels] + [hidden_channels_mlp] * num_mlp_layers + [2]
    mlp = build_mlp(layer_sizes, activation_fn, dropout_mlp)
...
with open(os.path.join(finetuning_results, "framework_complex_finetuning_free_neighbor.txt"), "w") as file:
    ...
    file.write(f"  PR-AUC Score val set: {trial.value}\n")
...
#         pruner=optuna.pruners.MedianPruner()
#     )
#     study.optimize(objective_framework_complex_gin_encoder, n_trials=NUMBER_OF_TRIALS, show_progress_bar=True)
#
#     # print and save the best trial
#     file.write("Best trial:\n")
#     trial = study.best_trial
